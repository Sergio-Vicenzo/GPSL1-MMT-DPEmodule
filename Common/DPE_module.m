...
% If MMT tracking is used, use MMT cost function for DPE
    for j=1:length(activeChnList)

         % Initialize parameters for MMT
        MMT_cost_func=zeros(1,length(chip_spacings));
        ...
        blksize = ceil((settings.codeLength-...
         ...
         caCode = generateCAcode(trackResults(activeChnList(j)).PRN);
         caCode1 = [caCode(end-1) caCode(end) caCode caCode(1) caCode(2)];
         ...
         parfor delay1_index=1:length(chip_spacings)
             delay1=chip_spacings(delay1_index)+LOS_delay;
            ...
            MMT_cost_func(delay1_index) = sum((iBasebandSignal-(LOS_ampl.*cos(LOS_carr_phase).*s_1)...
            -(NLOS_ampl.*cos(NLOS_carr_phase).*s_2)).^2) ...
            +  sum((qBasebandSignal-(LOS_ampl.*sin(LOS_carr_phase).*s_1)...
            -(NLOS_ampl.*sin(NLOS_carr_phase).*s_2)).^2);
...
if settings.DPE_plotCorrelogram == 1
    ...
    % === Plot correlogram for single satellite ===========================
        ...
        ax.MinorGridAlpha = 1;
        if settings.MMT == 1
             title(['PRN ', num2str(trackResults(activeChnList(j)).PRN), ' with MMT'],...
            ...
            % Save figure in PNG
             ...
             saveas(gcf, [pwd ,'\',settings.outfile_root,'\Correlogram\',...
                 settings.outfile_root,'_Correlogram_Epoch',num2str(currMeasNr),...
                 '_PRN',num2str(trackResults(activeChnList(j)).PRN),'_withMMT']);
        else
...
% === Plot correlogram from all satellites ================================
    ...
    ax.MinorGridAlpha = 1;
    if settings.MMT == 1
        title(['Epoch ', num2str(currMeasNr)], ['MMT-DPE 2D Positioning Error = ', ...
        ...
        % Save figure in MATLAB figure
        saveas(gcf, [pwd ,'\',settings.outfile_root,'\Correlogram\',...
         settings.outfile_root,'_Correlogram_Epoch',num2str(currMeasNr),'_withMMT']);
    else
